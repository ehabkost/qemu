...
inval:
    error_setg(errp, "invalid mac address: '%s'", str);
    g_free(str);
}
                       ...
                       void *opaque, Error **errp)
{
    ...
    queues = qemu_find_net_clients_except(str, peers,
                                          ...
                                          MAX_QUEUE_NUM);
    if (queues == 0) {
        error_setg(errp, "netdev '%s' not found", str);
        goto out;
    }
    ...
    for (i = 0; i < queues; i++) {
        if (peers[i]->peer) {
            error_setg(errp, "netdev '%s' is in use", str);
            goto out;
        }
                         ...
                         void *opaque, Error **errp)
{
    ...
    if (!state) {
        error_setg(errp, "audiodev '%s' not found", str);
        goto out;
    }
    card->state = state;
...
invalid:
    error_setg(errp, "invalid PCI address: '%s'", str);
    g_free(str);
}
...
inval:
    error_setg(errp, "invalid host device address: '%s'", str);
    g_free(str);
}
